...
struct GeminiAdvisor {
    func outfitNote(for weather: WeatherSnapshot, hourly: [HourlyForecast], language: AuraLanguage, apiKey: String) async throws -> String {
        ...
        let prompt = """
        You are a concise personal weather stylist. Based only on this local weather summary, give one practical outfit recommendation in \(language == .russian ? "Russian" : language == .german ? "German" : "English"). Keep it under 45 words. No greetings or emojis. Area: \(weather.city). Current condition: \(weather.condition). Temperature: \(Int(weather.temperature.rounded()))C, feels like \(Int(weather.feelsLike.rounded()))C, wind \(Int(weather.windSpeed.rounded())) km/h. Peak rain probability today: \(weather.rainChance)%. Next hours: \(nextHours). A probability is not rain happening now; mention this if rain is relevant.
        """
