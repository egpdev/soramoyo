...
private struct Sidebar: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 10) { LiquidMark().frame(width: 32, height: 32); Text("SORAMOYO").font(.system(size: 15, weight: .bold, design: .rounded)).tracking(2.2) }
            ...
            Spacer()
            VStack(alignment: .leading, spacing: 7) { Text(language.t("localWeather")).micro(); Text(language.t("localPrivacy")).muted() }.padding(28)
...
private struct NowView: View {
    ...
    var body: some View { ScrollView { VStack(alignment: .leading, spacing: 26) { Header(language: language); currentWeather; advice }.padding(42) } }
        ...
        ZStack(alignment: .topTrailing) {
            HStack(spacing: 28) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Label(language.condition(weather.snapshot.code), systemImage: weather.snapshot.symbol).font(.system(size: 16, weight: .medium)).foregroundStyle(Color.ice)
                    Text("\(language.t("high")) \(weather.snapshot.temperatureText(weather.snapshot.high, unit: unit))  ·  \(language.t("low")) \(weather.snapshot.temperatureText(weather.snapshot.low, unit: unit))").font(.system(size: 14, design: .monospaced)).foregroundStyle(.white.opacity(0.48))
                }
                Spacer(minLength: 20)
                Button {
                    withAnimation(.spring(response: 0.42, dampingFraction: 0.72)) { showsDayPulse.toggle() }
                } label: {
                    LiquidWeatherWidget(snapshot: weather.snapshot, unit: unit, language: language)
                        .frame(width: 250, height: 250)
                        .scaleEffect(showsDayPulse ? 1.035 : 1)
                        .shadow(color: Color.ice.opacity(showsDayPulse ? 0.34 : 0.18), radius: showsDayPulse ? 42 : 28)
                }.buttonStyle(.plain).accessibilityLabel(language.t("openToday"))
            }
            if showsDayPulse {
                DayPulse(snapshot: weather.snapshot, hours: weather.hourlyForecast, unit: unit, language: language) {
    ...
    private var advice: some View {
        let local = language.localOutfit(weather.snapshot)
        return VStack(alignment: .leading, spacing: 17) {
            HStack(alignment: .top, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 7) { Text(language.t(weather.geminiNote == nil ? "whatWear" : "aiNote")).micro(); Text(weather.geminiNote ?? local.headline).font(.system(size: 22, weight: .medium, design: .rounded)); if weather.geminiNote == nil { Text(local.detail).font(.system(size: 14)).foregroundStyle(.white.opacity(0.57)) } }
                Spacer(); if weather.geminiNote == nil { VStack(alignment: .trailing, spacing: 8) { ForEach(local.items, id: \.self) { Tag(text: $0) } } }
            }
            Divider().overlay(Color.white.opacity(0.09))
            HStack { Text(language.geminiStatus(weather.geminiStatus)).font(.system(size: 12)).foregroundStyle(.white.opacity(0.44)); Spacer(); if weather.hasGeminiKey { Button(language.t("askGemini")) { weather.generateGeminiAdvice(language: language) }.buttonStyle(AuraButtonStyle()) } else { Button(language.t("enableGemini")) { openSettings() }.buttonStyle(AuraButtonStyle()) } }
...
private struct DayPulse: View {
    ...
    private var summary: String {
        let rain = language.t(snapshot.rainChance >= 35 ? "rainLikely" : "rainUnlikely")
        return "\(language.t("mildNow")) \(snapshot.temperatureText(snapshot.high, unit: unit)). \(rain)"
    }

    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack { Text(language.t("dayPulse")).micro(); Spacer(); Button(action: dismiss) { Image(systemName: "xmark").font(.system(size: 10, weight: .bold)).frame(width: 24, height: 24).background(.white.opacity(0.10), in: Circle()) }.buttonStyle(.plain) }
            ...
            if visibleHours.isEmpty {
                Text(language.t("hourlyEmpty")).muted()
            } else {
                ForEach(visibleHours) { hour in
                    HStack(spacing: 10) {
                        Text(hour.date.formatted(.dateTime.hour(.twoDigits(amPM: .omitted)).locale(language.locale))).font(.system(size: 11, design: .monospaced)).frame(width: 42, alignment: .leading).foregroundStyle(.white.opacity(0.5))
                        Image(systemName: hour.symbol).font(.system(size: 13)).foregroundStyle(Color.ice).frame(width: 17)
                        Text(language.condition(hour.code)).font(.system(size: 12)).foregroundStyle(.white.opacity(0.7))
                        Spacer()
...
private struct ForecastView: View {
    ...
    var body: some View { ScrollView { VStack(alignment: .leading, spacing: 26) {
        Header(title: language.t("forecast"), subtitle: language.t("weekAhead"), language: language)
        HStack(spacing: 12) { ForEach(days) { ForecastCell(day: $0, unit: unit, language: language) } }
        ...
        Text(language.t("forecastRefresh")).muted()
    }.padding(42) } }
...
private struct SettingsView: View {
    ...
    var body: some View { ScrollView { VStack(alignment: .leading, spacing: 23) {
        Header(title: language.t("settings"), subtitle: language.t("makeYours"), language: language)
        ...
        VStack(alignment: .leading, spacing: 15) { Text(language.t("units")).micro(); Picker(language.t("temperature"), selection: $unitRaw) { ForEach(TemperatureUnit.allCases) { Text($0.label).tag($0.rawValue) } }.pickerStyle(.segmented) }.padding(24).glassPanel()
        VStack(alignment: .leading, spacing: 15) { Text(language.t("language")).micro(); Picker(language.t("language"), selection: $languageRaw) { ForEach(AuraLanguage.allCases) { Text($0.label).tag($0.rawValue) } }.pickerStyle(.segmented) }.padding(24).glassPanel()
        VStack(alignment: .leading, spacing: 13) { Text(language.t("geminiTitle")).micro(); Text(language.t("geminiPrivacy")).muted(); HStack { SecureField(weather.hasGeminiKey ? language.t("keySaved") : language.t("pasteKey"), text: $key).textFieldStyle(.plain).padding(12).background(.white.opacity(0.08), in: RoundedRectangle(cornerRadius: 10)); Button(language.t("saveKey")) { weather.saveGeminiKey(key); key = "" }.buttonStyle(AuraButtonStyle()).disabled(key.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) }; Text(language.geminiStatus(weather.geminiStatus)).font(.system(size: 12)).foregroundStyle(.white.opacity(0.44)) }.padding(24).glassPanel()
        HStack { Spacer(); Button(language.t("backNow")) { openNow() }.buttonStyle(AuraSecondaryButtonStyle()) }
    }.padding(42) } }
}
...
private struct LiquidWeatherWidget: View { let snapshot: WeatherSnapshot; let unit: TemperatureUnit; let language: AuraLanguage; var body: some View { ZStack { RoundedRectangle(cornerRadius: 48, style: .continuous).fill(.ultraThinMaterial).opacity(0.55).overlay(RoundedRectangle(cornerRadius: 48).stroke(.white.opacity(0.22), lineWidth: 1)); Circle().fill(Color.ice.opacity(0.15)).blur(radius: 25).frame(width: 150); LiquidMark().frame(width: 112, height: 112); VStack { HStack { Text("空模様").micro(); Spacer(); Text(language.t("live")).micro() }.foregroundStyle(.white.opacity(0.48)).padding(20); Spacer(); Text(snapshot.temperatureText(snapshot.temperature, unit: unit)).font(.system(size: 27, weight: .medium, design: .rounded)).padding(.bottom, 20) } }.shadow(color: Color.ice.opacity(0.19), radius: 34) } }
